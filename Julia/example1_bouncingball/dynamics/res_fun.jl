...
function res_fun(u, params, ODESettings)
    ...
    f1(dx, x, xi, t) = f(dx, x, xi, t, params) # flight (phase 1)
    x0 = x1_bar
    if t1 == 0
        x1   = x0
        Phi1 = I(n)
    ...
    f2(dx, x, xi, t) = f(dx, x, xi, t, params) # flight (phase 2)
    x0 = x2_bar
    if t2 == 0
        x2   = x0
        Phi2 = I(n)
    ...
    # Compute reset maps
    delta_td_(x) = delta_td(x, params)
    x1_P = delta_td_(x1)
    D12  = ForwardDiff.jacobian(delta_td_, x1)

    # Compute events and anchor
    e_td_(x) = e_td(x, [], [], params)
    e12  = e_td_(x1)
    de12 = ForwardDiff.gradient(e_td_, x1)'
